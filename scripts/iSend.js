...
$(function() {
	 ...
	 */
	var url = {
		time_chart: "doPerson/tuhaoChart.php",
		getMore: 'doPerson/newGoods.php'
	}
	$('.week-chart,.month-chart').on('click', function(event) {
			...
			if (target.hasClass('month-chart')) {
				date = 'month';
				$('.chart>a').attr('href', 'tuhao-chart.php');
			} else if (target.hasClass('week-chart')) {
				date = 'week';
				$('.chart>a').attr('href', 'tuhao-chart.php?type=week');
			} else {
				date = 'month';
				$('.chart>a').attr('href', 'tuhao-chart.php');
			}
	...
	function joint(data) {
		...
		for (var i = 0; i < data.length; i++) {
			...
			li =
				'<a href=detail.php?pro_id=' + _data.id + '>' +
				'<div class="thumbnail" >' +
				'<img src=' + _data.src + ' />' +
				'</div></a><div class="sticky-note" >' +
				'<a href=detail.php?pro_id=' + _data.id + '>ABOUT MORE</a>' +
				'<p>' + _data.pro_desc + '</p>' +
