...
//			console.log({pro_id:pro_id,receiver_id:receiver_id});
			$.post('doPerson/giveWho.php',{pro_id:pro_id,receiver_id:receiver_id},function(res){
//				res = $.parseJSON(res);
				if(res.code==1){
...
//					res = $.parseJSON(res);
		...
		$('#edit-goods').on('click',function(e) {
			...
			window.location.href = './edit.php?pro_id=' + proId;
		});
...
//              console.log(user_id)
	...
	function getCmt(curPage){
		$.get('doPerson/goodsComm.php',{curPage:curPage,pro_id:pro_id},function(res){
//			res = $.parseJSON(res);
			if(res.code==1){
				if (res.maxPage==0) {
					pages.max = 1;
