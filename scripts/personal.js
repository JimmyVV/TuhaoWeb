...
var jcrop_api, boundx, boundy, $pcnt, canvas, image, ctx,
	Pathurl = {
		getInfo: 'doPerson/editUser.php?active=get',  //请求得到基本信息,姓名，学院
		changeInfo:'doPerson/editUser.php?active=edit'  //修改提交，姓名，学院, 返回成功失败
	}
	...
	var Edit = {
		...
		showDetail: function() {
			...
			$.ajax({
				...
				_this.info.val(data.name);
//				$('#editColleges .option[data-value="'+data.department+'"]').addClass('active')
				_this.colleges.attr('data-value',data.department);
				_this.colleges.text(data.department);
		...
		finishEdit: function() {
			...
			this.finish_edit.on('click', function() {
				var name = _this.info.val().trim(),
					...
					address = _this.address.attr('data-value').trim(); //楼栋
...
function createMesList(data) {
	...
	$text.find('a').attr('href', 'detail.php?pro_id=' + data.pro_id);
...
function getGoods(curPage, tab) {
	var url = "doPerson/sent.php";
	if (tab == 2) {
		url = "doPerson/received.php"
	} else if (tab == 3) {
		url = "doPerson/sending.php"
	}
	$.ajax({
		type: "get",
		url: url,
