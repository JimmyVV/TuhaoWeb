define(function(require) {
	...
	$(function() {
		...
		function submitSearch() {
			...
			if (content) {
				window.location.href = 'search.php?key=' + content;
			}
		}
		...
		var afterlogin = function() {
			...
			$("[role='release']").off("click").on("click", function() {
				$(this).find("a").attr('href', "issue.php");
			});
			...
			$('.item1 li[role="sending"]>a').attr("href", "personal.php?type=goods");
		}
		...
		$("#signin").find("[role='nickname']").on("blur", function() {
			...
			if (!content) {
				...
				// 执行ajax操作
				$.ajax({
					url: "doAction.php?action=registerName",
					type: "post",
					data: {
						nickname: content
					},
					success: function(data) {
						signment = data.success;
						if (!data.success) {
		...
		$("#signin").find("[role='email']").on("blur", function() {
			...
			if (content == "" || null) {
				...
				// 执行ajax操作
				$.ajax({
					url: "doAction.php?action=registerEmail",
					type: "post",
					data: {
						email: content
					},
		...
		var addString = function(text, ele) {
			...
			$('#username').addClass('content-span').html('<a href="personal.php">' + text + '</a>');
		...
		$("#info").find("[name='submit']").on("click", function(event) {
			...
			if (!((sexFlag && collegeFlag) && (yearFlag && addressFlag))) {
				...
				$.ajax({
					url: "doAction.php?action=register",
					type: "post",
					data: {
						"nickname": nickname,
						"email": email,
					...
					success: function(data) {
						...
						if (!data.success) {
							...
							$.modal.close();
							window.location.href = "reset.php?action=register-ok";
						}
					},
					error: function() {
						$this.removeClass('disabled');
			...
			if (_password === "" || _password == null || _email === "" || _email == null) {
				...
				$.ajax({
					...
					success: function(data) {
						...
						switch (data.success) {
							case 1:
								window.location.href = "reset.php?action=remind";
								break;
							case 2:
								$prompt.text("");
								$.modal.close();
								afterlogin();
								addString(data.mes, $(".logo-self-info"));
		...
		$('#login .forgetps-btn').on('click', function() {
			window.location.href = 'reset.php?action=forget';
		})
	})
})