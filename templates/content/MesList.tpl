<div class="list">
	<header>
		...
		<ul class="mes" >
			<li><a class="active" href="javascript:void(0);" data-tab="4">我收到的留言{if $mesNum } ({$mesNum}){/if}</a></li>
		</ul>
	</header>
	<ul>
		{foreach $data as $item}
		<li class="mes {if $item.status == 0} new {/if}">
			<div class="mes_title">
			...
			<div class="textWrapper">
				<div class="text" data-id="{$item.id}" data-parent="{$item.parent_id}">
					<a href="detail.php?pro_id={$item.pro_id}">{$item.content}</a>
				</div>
